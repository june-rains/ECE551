...
wordarray_t * replaceAll(wordarray_t * arr,
  ...
  for (size_t i = 0; i < catarr->n; i++) {
    ...
    char * endptr1;
    char * endptr2;
    if (strtol(catarr->wordArr[i], &endptr1, 10) == 0) {
      if (endptr1 == catarr->wordArr[i]) {
        ...
        fprintf(stderr, "Invalid format of the category number!");
        exit(EXIT_FAILURE);
      }
    }
    /***************CATEGORY WITH VALID REFERENCE NUMBER***********/
    else if (strtol(catarr->wordArr[i], &endptr2, 10) >= 1 &&
             strtol(catarr->wordArr[i], &endptr2, 10) <= (int)(*usedWords)->n_words) {
      if (strcmp(endptr2, "\0") == 0) {
        new =
            strdup((*usedWords)->words[(*usedWords)->n_words - atoi(catarr->wordArr[i])]);
        (*usedWords)->words =
            realloc((*usedWords)->words,
                    ((*usedWords)->n_words + 1) * sizeof(*(*usedWords)->words));
        (*usedWords)->words[(*usedWords)->n_words] = strdup(new);
