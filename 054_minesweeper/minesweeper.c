...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  //Initialize/malloc the struct
  board_t * answer = malloc(sizeof(*answer));
  answer->width = w;
  answer->height = h;
  answer->totalMines = numMines;
  answer->board = malloc(answer->height * sizeof(*answer->board));
  for (int i = 0; i < answer->height; i++) {
    answer->board[i] = malloc(answer->width * sizeof(*answer->board[i]));
  }

  //Fill in UNKNOWN
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      answer->board[i][j] = UNKNOWN;
    }
  }

  //add mines
  for (int i = 0; i < numMines; i++) {
    addRandomMine(answer);
  }
